...
def box_plot(df, cat_var, cont_var):
    ...
    return px.box(df, x=cat_var, y=cont_var)
...
def scatterplot(df, xvar, yvar):
    ...
    return px.scatter(df, x=xvar, y=yvar)
...
def scatterplot_groups(df, xvar, yvar, groups):
    ...
    return px.scatter(df, x=xvar, y=yvar, color=groups)
...
def scatterplot_matrix(df, numeric_cols):
    ...
    return px.scatter_matrix(df, dimensions=numeric_cols)


def bar_chart_means(df, cat_var, continuous_var, labels):
    grouped = df.groupby(cat_var, as_index=False)[continuous_var].mean()
    fig = px.bar(
        grouped,
        x=cat_var,
        y=continuous_var,
...
def stacked_bar_counts(df, cat_var_1, cat_var_2, labels):
    ...
    which summarises the mean of the continuous variable by different levels
    of cat_var and labels the axes using the labels dict."""
    counts = df.groupby([cat_var_1, cat_var_2]).size().reset_index(name="count")
    return px.bar(counts, x=cat_var_1, y="count", color=cat_var_2, labels=labels)
